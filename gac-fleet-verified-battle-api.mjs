...
import { gacFleetVerifiedBattleService } from "./gac-fleet-verified-battle-service.mjs";
...
export function createGacFleetVerifiedBattleApi(options = {}) {
  ...
  const battles = options.battles || gacFleetVerifiedBattleService;
  ...
  async function currentContext(code, requestedRoundInput) {
    ...
    if (!id) {
      const error = new Error("No current GAC event is available for verified fleet battle capture.");
      error.status = 404;
      throw error;
    }
    ...
    if (!round) {
      const error = new Error("Select the current GAC round before verifying a fleet battle result.");
      error.status = 400;
      throw error;
    }
    const confirmed = await confirmation.findLatestConfirmed(code, id, round);
    if (!confirmed?.opponent?.allyCode) {
      const error = new Error("Confirm the current opponent before verifying fleet battle history.");
      error.status = 409;
      throw error;
    }
  ...
  return Object.freeze({
    async handle(request, response, url) {
      const match = request.method === "POST" && url.pathname.match(/^\/api\/gac\/fleet-verified-battle\/(\d{9})$/);
      if (!match) return false;
      try {
        assertSameOrigin(request);
        const user = await authSession.currentUser(request);
        if (!user?.id) {
          const error = new Error("Sign in with the verified owner account before archiving a fleet battle result.");
          error.status = 401;
          throw error;
        }
        ...
        writeJson(response, 200, result, {
          ...
          "X-GAC-Fleet-Battle-Evidence": "verified-owner-explicit-confirmation",
        });
      } catch (error) {
        writeJson(response, statusFor(error), { error: error?.message || "The fleet battle result could not be verified." });
      }
      return true;
    },
  });
