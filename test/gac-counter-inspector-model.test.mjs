...
test("primary evidence provenance requires the exact saved defense composition", () => {
  const own = { units: [unit("PRIMARY_LEAD"), unit("PRIMARY_2"), unit("PRIMARY_3")] };
  const exact = primaryEvidenceMatch(
    own,
    defense,
    { observations: [observation("PRIMARY")] },
    ["PRIMARY_3", "PRIMARY_LEAD", "PRIMARY_2"],
    { size: 3 },
  );
  assert.ok(exact);
  ...
  const variant = primaryEvidenceMatch(
    own,
    defense,
    { observations: [observation("PRIMARY", { enemyMembers: ["DEF_LEAD", "OTHER_2", "OTHER_3"] })] },
...
test("recovery alternates are pairwise non-overlapping and avoid protected resources", () => {
  ...
  const enemyUnits = [unit("DEF_LEAD", { faction: "Enemy" }), unit("DEF_2", { faction: "Enemy" }), unit("DEF_3", { faction: "Enemy" })];
  const evidence = {
    observations: [
...
test("roster resolution and observed-rate helpers preserve unknown evidence", () => {
