...
const base = {
  ...
  options: Array.from({ length: 23 }, (_, index) => ({ type: 1, name: `existing-${index + 1}`, description: `Existing ${index + 1}` })),
};

test('Stage 10 uses exactly the two remaining /tb subcommand slots', () => {
  const result = applyDiscordStage10TbCommandSchema(base);
  assert.equal(result.changed, true);
  assert.deepEqual(result.added, ['plan-delivery', 'delivery-status']);
  ...
  assert.deepEqual(delivery.options.find((row) => row.name === 'action').choices.map((row) => row.value), ['preview', 'publish']);
  ...
  const status = result.command.options.find((row) => row.name === 'delivery-status');
  assert.deepEqual(status.options.map((row) => row.name), ['phase', 'version']);
  assert.equal(status.options.every((row) => row.required === true), true);
...
test('Stage 10 refuses to exceed Discord 25-subcommand limit', () => {
  const full = { ...base, options: [...base.options, { type: 1, name: 'already-24', description: '24' }] };
...
test('Stage 10 schema has an explicit version identifier', () => {
  assert.match(DISCORD_STAGE10_DELIVERY_SCHEMA_VERSION, /stage10-controlled-delivery-v1$/);
});
