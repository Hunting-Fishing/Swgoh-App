...
const catalog = [
  { baseId: "GL", name: "Legend", unitType: "Character", categories: ["galactic_legend"] },
...
test("filters and sorts guild members for officer roster review", () => {
  ...
  const formattedAlly = filterGuildMembers(snapshot.members, { search: "444-555-666", status: "All", sort: "name" });
  assert.equal(formattedAlly.length, 1);
  assert.equal(formattedAlly[0].name, "Bravo");
  const unavailable = filterGuildMembers(snapshot.members, { status: "Unavailable", sort: "name" });
  assert.deepEqual(unavailable.map((row) => row.name), ["Charlie"]);
});
