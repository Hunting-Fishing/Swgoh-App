...
function canonicalFixture() {
  const member={id:'game-player-1',playerId:'game-player-1',persistentId:playerId,allyCode,name:'Warm Bacon',galacticPower:12_000_000,rosterAvailable:true};
  const units=[
    {baseId:'AAYLASECURA',name:'Aayla Secura',unitType:'Character',stars:7,gear:13,relic:2,power:25_000,factions:['Jedi','Galactic Republic'],tags:['Jedi','Galactic Republic']},
    {baseId:'PLOKOON',name:'Plo Koon',unitType:'Character',stars:7,gear:13,relic:3,power:24_000,factions:['Jedi','Galactic Republic'],tags:['Jedi','Galactic Republic']},
  ];
  const catalog=units.map((unit)=>({baseId:unit.baseId,name:unit.name,unitType:'Character',alignment:'Light Side',categories:unit.tags,factions:unit.factions}));
  return {
    ...
    async getPlayerRoster(code){assert.equal(code,allyCode);return {fetchedAt:'2026-08-19T00:00:00Z',player:{allyCode,name:'Warm Bacon'},units,ships:[]};},
...
test('TB Farm Plan executes and persists for an active verified Guild member without calling Discord', async () => {
  ...
  let discordCalls=0;
  const service=createWebActionService({store,canonical:canonicalFixture(),env:{},fetch:async()=>{discordCalls+=1;throw new Error('Discord must not run during website execution');},now:()=>new Date('2026-08-19T00:10:00Z')});
