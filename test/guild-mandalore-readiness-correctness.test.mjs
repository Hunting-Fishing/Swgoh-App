...
test('third Mandalorian excludes required core units and selects the strongest valid candidate', () => {
  const report = buildMandaloreMemberReadiness(member(1, [
    { baseId: 'MANDALORIANLOW', name: 'Low Mando', gear: 13, relic: 5, power: 30000, categories: ['affiliation_mandalorian'] },
    { baseId: 'MANDALORIANHIGH', name: 'High Mando', gear: 13, relic: 8, power: 41000, categories: [{ id: 'affiliation_mandalorian' }] },
  ]));
...
test('catalog-provided Mandalorian tags are merged onto compact persisted units', () => {
  const compactMember = member(2, [{ baseId: 'BO-KATAN', name: 'Bo-Katan Kryze', gear: 13, relic: 7, power: 36000 }]);
  const catalog = [{ baseId: 'BO-KATAN', name: 'Bo-Katan Kryze', categories: ['affiliation_mandalorian'] }];
...
test('guild summary separates eligibility potential from the required 25 successful clears', () => {
  const members = Array.from({ length: 24 }, (_, index) => member(index + 1, [
    { baseId: `MANDO${index}`, name: `Mando ${index}`, gear: 13, relic: 7, power: 35000, categories: ['affiliation_mandalorian'] },
  ]));
