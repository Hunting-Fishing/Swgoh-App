...
const EXPECTED_3V3 = {
  carbonite: { 'FRONT-TOP': 1, 'FRONT-BOTTOM': 1, 'BACK-BOTTOM': 2, 'BACK-TOP': 1 },
...
test('5v5 league capacities match the supplied GAC defense table', () => {
...
test('3v3 league capacities match the supplied GAC defense table', () => {
  ...
  assert.equal(leagueBoard('3v3', 'carbonite').squadCount, 4);
  assert.equal(leagueBoard('3v3', 'carbonite').totalPlacements, 5);
});
...
test('live league UI uses exact circular slot targeting and preserves hidden source cards', () => {
  ...
  const layout = fs.readFileSync(path.join(root, 'public/gac-live-arena-layout.css'), 'utf8');
  ...
  assert.match(ui, /gac-league-card-storage/);
  assert.match(ui, /gac-live-selected/);
  assert.match(ui, /portraitMarkup\(portraits\[0\], 'is-leader'\)/);
  assert.match(css, /gac-league-node-orbit/);
  assert.match(css, /border-radius:50%/);
  assert.match(css, /gac-league-node-pips/);
  assert.match(layout, />\.gac-manual-gac-map/);
