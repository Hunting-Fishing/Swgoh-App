...
test('adds plan-status exactly once without changing existing TB subcommands', () => {
  const first = applyDiscordStage9TbCommandSchema(base);
  assert.equal(first.changed, true);
  assert.deepEqual(first.added, ['plan-status']);
...
test('Stage 9 command schema has its own explicit version identifier', () => {
  assert.match(DISCORD_STAGE9_PLAN_SCHEMA_VERSION, /stage9-plan-status-v1$/);
});
