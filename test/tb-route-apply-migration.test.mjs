...
test('route apply migration creates immutable fingerprinted phase snapshots', () => {
  ...
  assert.match(sql, /revoke update, delete, truncate on table public\.guild_tb_phase_snapshots from service_role/i);
  assert.match(sql, /grant select, insert on table public\.guild_tb_phase_snapshots to service_role/i);
  assert.doesNotMatch(sql, /grant all on table public\.guild_tb_phase_snapshots to service_role/i);
});

test('atomic route apply RPC guards event phase, duplicate zones, zone versions and officer locks', () => {
  const sql = fs.readFileSync(sqlUrl, 'utf8');
  ...
  assert.match(sql, /count\(distinct u->>'planetId'\)/i);
  assert.match(sql, /TB_ROUTE_DUPLICATE_ZONE_UPDATE/);
