...
export function createGuildDiscordAdminApi(options = {}) {
  ...
  async function handle(request, response, url) {
    ...
    try {
      ...
      if (request.method === 'GET' && action === 'links') { writeJson(response, 200, await links.list(user.id, code)); return true; }
      ...
      if (action === 'link-member') { writeJson(response, 200, await links.link(user.id, code, { discordUserId: body?.discordUserId, swgohAllyCode: body?.swgohAllyCode })); return true; }
      if (action === 'unlink-member') { writeJson(response, 200, await links.unlink(user.id, code, { discordUserId: body?.discordUserId })); return true; }
