...
function namedStatuses(text, names) {
...
export function extractAbilitySemantics(ability = {}) {
  ...
  const buffs = namedStatuses(description, BUFFS);
  const debuffs = namedStatuses(description, DEBUFFS);
  const conditions = unique(mechanics.filter((item) => item.kind.startsWith("on_") || item.kind.startsWith("start_") || item.kind.startsWith("end_") || item.kind === "threshold_health").map((item) => item.kind));
  return {
    schemaVersion: 1,
