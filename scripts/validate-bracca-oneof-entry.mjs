...
import { braccaBattleStrategyForMission } from "../public/tb-battle-strategy-bracca-data.js";
...
const strategy = braccaBattleStrategyForMission("bracca-zeffo-unlock");
assert.ok(strategy, "Bracca battle strategy pack missing");
assert.ok(strategy.sources.some((source) => source.kind === "official"), "Bracca strategy needs official entry evidence");
assert.ok(strategy.sources.some((source) => source.kind === "community-tested" || source.kind === "community-reference"), "Bracca strategy needs scoped encounter evidence");
assert.ok(strategy.failureRisks.some((risk) => /win probability|guarantee|guaranteed/i.test(risk)), "Bracca strategy must explicitly reject imported guarantee claims");
...
assert.equal(strategyCal.status, "ready", "Original Cal legal variant must not be penalized by JKCK-only advice");
...
assert.equal(strategyJkck.status, "ready", "JKCK legal variant should load the same source-scoped strategy pack");
assert.equal("winPercent" in strategyJkck, false, "Bracca strategy must not invent win probability");

console.log("[bracca-oneof] validated Cere + one-of Cal contract and source-scoped battle strategy");
