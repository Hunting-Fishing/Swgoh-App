...
async function sync() {
  ...
  console.log(`[raw-combat] syncing skill/ability/effect graph for ${catalog.gameVersion || "unknown"}`);

  const [skills, abilities, effectsCompressed, enums] = await Promise.all([
  ...
  const gameVersion = assertVersionCompatibility(catalog, [skills, abilities, effects, enums]);
  ...
  console.log(`[raw-combat] linked ${index.coverage.linkedAbilities}/${index.coverage.totalAbilities} abilities (${index.coverage.linkedPercent}%)`);
}

sync().catch(async (error) => {
  if (ALLOW_STALE && await exists(OUTPUT_PATH)) {
    console.warn(`[raw-combat] refresh failed; preserving existing effect graph: ${error?.message || error}`);
    return;
  }
