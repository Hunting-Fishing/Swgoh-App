...
import {
  ...
  DISCORD_STAGE10_TB_SUBCOMMANDS,
...
async function discordRequest(pathname, options = {}) {
    ...
    try {
      const response = await fetch(`https://discord.com/api/${API_VERSION}${pathname}`, {
        ...
        headers: {
          ...
          'User-Agent': `SWGOH-Command-Center (stage10-delivery-schema; ${DISCORD_STAGE10_DELIVERY_SCHEMA_VERSION})`,
        },
      ...
      const error = new Error(`Discord Stage 10 schema request failed with HTTP ${response.status}.`);
  ...
  throw lastError || new Error('Discord Stage 10 schema request failed.');
}

function missingStage10Subcommands(options = []) {
  const returned = new Set((Array.isArray(options) ? options : []).map((option) => String(option?.name || '').toLowerCase()));
  return DISCORD_STAGE10_TB_SUBCOMMANDS.map((row) => row.name).filter((name) => !returned.has(name));
}

async function patchStage10Schema() {
  ...
  if (!tb?.id) throw new Error('Registered guild-scoped /tb command was not found after Stage 9 command patching.');

  const patch = applyDiscordStage10TbCommandSchema(tb);
  if (!patch.changed) {
...
if (!config.commandRegistrationConfigured) {
  const message = 'Stage 10 Discord schema patch requires DISCORD_APPLICATION_ID, DISCORD_BOT_TOKEN, and DISCORD_DEFAULT_GUILD_ID.';
  if (ifConfigured) console.log(`Skipping Stage 10 Discord schema patch: ${message}`);
  else {
  ...
  try {
    await patchStage10Schema();
  } catch (error) {
    const message = error?.message || 'Discord Stage 10 schema patch failed.';
    console.error(message);
