...
function resolveRoteContext(button) {
  ...
  const recommendation = mission.recommendations?.[index] || null;
  if (!recommendation) return null;
  const baseIds = (recommendation.members || []).map((member) => String(member.baseId || "")).filter(Boolean);
  return {
    planetId,
    planetName: String(map.planetName || planetId),
    nodeId,
    mission,
    recommendation,
...
function renderContext() {
  ...
  panel.innerHTML = `
    ...
    <div class="squad-rote-boundary"><strong>Evidence boundary:</strong> ${assessment.evidence === "exact" ? "entry legality is evaluated from verified mission restrictions." : "the full selectable-unit rule is not encoded, so this panel reports known gates only."} ${state.context.recommendation?.verifiedLegal ? "The loaded source template is marked verified-legal." : "The loaded source is a planning template unless explicitly marked verified-legal."} Strategy coverage is separate and does not make this a guaranteed-win team.</div>
...
function install() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest?.("[data-squad-rote-clear]")) {
      clearContext();
    }
  }, true);
