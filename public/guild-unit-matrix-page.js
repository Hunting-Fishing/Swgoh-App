...
function plannerControls() {
  ...
  return {
    ...
    preferences: asArray(safety?.preferences).filter((row) => row?.memberId && row?.baseId && ["give", "keep"].includes(normalizeDonationPreference(row.preference))),
    ignoredMembers: asArray(safety?.ignoredMembers),
  };
}
