...
function tbFarmPlanItem(item) {
  ...
  const goals = Array.isArray(personal.trackedGoals) ? personal.trackedGoals : [];
  return `<article class="web-action-feed-item"><div class="web-action-feed-item-head"><div><div class="web-action-feed-kicker">TB FARM PLAN · ROTE${personal.trackedGoalCount?' · PERSONALIZED':''}</div><h4>${escapeHtml(result.player?.name || 'Shared TB Farm Plan')}</h4><small>${escapeHtml(result.guild?.name || '')}${goals.length?` · goals: ${escapeHtml(goals.slice(0,4).map((goal)=>goal.shortName||goal.name).join(', '))}${goals.length>4?'…':''}`:''}</small></div><div><div class="web-action-feed-score">${personal.trackedGoalCount?`${Number(summary.trackedGoalRows||0)} my-goal farms`:`${Number(summary.doubleUseRows||0)} double-use farms`}</div><div class="web-action-feed-time">${escapeHtml(dateTime(item.publishedAt || item.run.createdAt))}</div></div></div><div class="web-action-feed-attempts">${recommendations.slice(0,6).map((row)=>{const personalCount=Number(row.personal?.trackedOverlapCount||0);return `<span class="web-action-feed-chip">#${Number(row.rank||0)} ${personalCount?'★ MY GOAL · ':''}${escapeHtml(row.unitName||row.baseId)} · ${escapeHtml(row.tbTargetLabel||'')} · ${Number(row.tb?.missionImpact||0)} TB mission${Number(row.tb?.missionImpact||0)===1?'':'s'}${personalCount?` · ${personalCount} tracked path${personalCount===1?'':'s'}`:Number(row.journey?.activeOverlapCount||0)>0?` · ${Number(row.journey.activeOverlapCount)} Journey overlap`:''}</span>`;}).join('')}</div></article>`;
}
