...
function rowMarkup(row){
  ...
  return `<article class="gac-relic-row is-${escapeAttr(band)}"><header><div>${portrait(index,row.defenseLeaderBaseId)}<span><b>${escapeHtml(row.defenseName)}</b><small>entered defense</small></span></div><strong>RΔ ${formatRelicDelta(row.relicDelta)}</strong><div>${portrait(index,row.counterLeaderBaseId)}<span><b>${escapeHtml(row.counterName)}</b><small>proposed counter</small></span></div></header><div class="gac-relic-metrics"><span><b>${relic(row.defenderAverageRelic)}</b> defense avg</span><span><b>${relic(row.attackerAverageRelic)}</b> counter avg</span><span><b>${pct(row.winRate)}</b> historical win</span><span><b>${number.format(row.battles||0)}</b> battles</span></div><footer><i>${escapeHtml(band.replaceAll('-',' ').toUpperCase())}</i><small>RΔ uses current loaded rosters. Win rate is separate historical evidence and is not relic-normalized.</small></footer></article>`;
}
...
function render(){
  ...
  root.innerHTML=`<header><div><span>CURRENT RELIC SUITABILITY</span><strong>Your counter relics vs the entered defense</strong><small>Current RΔ is a roster-fit check. It is intentionally separate from historical win-rate evidence.</small></div><div>${state.analyzedKey?`<button type="button" data-gac-relic-toggle>${state.open?'HIDE':'VIEW · '+state.rows.length}</button>`:''}<button type="button" data-gac-relic-analyze ${state.loading?'disabled':''}>${state.loading?'ANALYZING…':state.analyzedKey?'REANALYZE':'ANALYZE RELIC FIT'}</button></div></header>${state.error?`<div class="gac-relic-error">${escapeHtml(state.error)}</div>`:''}${state.open&&state.analyzedKey?`<div class="gac-relic-note">Historical counter evidence in this build is not yet normalized by historical relic snapshots. These deltas compare the rosters loaded right now.</div><div class="gac-relic-grid">${state.rows.length?state.rows.map(rowMarkup).join(''):'<div class="gac-relic-empty">No complete non-overlapping counter allocation is currently available.</div>'}</div>`:''}`;
}
