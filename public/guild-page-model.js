...
function isGalacticLegend(unit = {}) {
  return categoryNames(unit).some((name) => name.toLowerCase().includes("galactic legend"));
}
...
export function filterGuildMembers(members = [], options = {}) {
  ...
  let rows = asArray(members).filter((member) => {
    ...
    return [member.name, member.allyCode, member.playerId, ...asArray(member.galacticLegends).map((row) => row.name)].join(" ").toLowerCase().includes(search);
  });
