-- Expose guild rank and player profile identity already captured by the canonical guild sync.
-- This is a read-shape upgrade only: no new table/column and no additional upstream requests.
...
member_rows as (
  select
    ...
    greatest(0, coalesce(nullif(gmc.metadata->>'memberLevel', '')::integer, 0)) as member_level,
    case greatest(0, coalesce(nullif(gmc.metadata->>'memberLevel', '')::integer, 0))
      when 4 then 'Guild Leader'
      when 3 then 'Officer'
      when 2 then 'Member'
      else null
    end as member_role,
    nullif(p.metadata->>'playerTitle', '') as profile_title,
    nullif(p.metadata->>'playerPortrait', '') as player_portrait,
    gmc.last_synced_at,
    exists (
  ...
  select
    ...
    coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'playerPortrait', player_portrait,
        'rosterAvailable', roster_available,
        'lastSyncedAt', last_synced_at,
        'units', units
      )
      order by
        case member_role when 'Guild Leader' then 0 when 'Officer' then 1 else 2 end,
        galactic_power desc,
        member_name
    ), '[]'::jsonb) as members
  from member_rows
...
select jsonb_build_object(
  ...
  'persistence', jsonb_build_object(
    ...
    'memberIdentityShape', 'guild-rank-profile-v1'
  )
)
from target_guild g
...
comment on function public.read_persisted_guild_roster(text) is
  'Returns the canonical persisted Guild roster plus captured guild rank, selected title and player portrait identity. Owned units remain compact progression data and static assets remain browser-cached.';
