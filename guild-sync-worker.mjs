...
function configFromEnv(env = process.env) {
  ...
  return Object.freeze({
    ...
    jobTimeoutMs: positiveInteger(env.GUILD_SYNC_JOB_TIMEOUT_MS, 600_000, 30_000, 900_000),
  });
}
function sleep(ms) { return new Promise((resolve) => setTimeout(resolve, ms)); }
function errorText(error) { return clean(error?.message || error || 'Guild sync worker failed.').slice(0, 1000); }
function timeoutError(ms) { const error = new Error(`Guild synchronization exceeded the worker hard limit of ${Math.round(ms / 1000)} seconds.`); error.code = 'GUILD_SYNC_JOB_TIMEOUT'; return error; }
...
export function createGuildSyncWorker(env = process.env, options = {}) {
  ...
  function status() {
    return Object.freeze({
      enabled: config.enabled, workerId: config.workerId, pollMs: config.pollMs, batchSize: config.batchSize,
  ...
  async function recoverStale() {
    const value = await store.rpc('recover_stale_guild_sync_jobs', { p_stale_seconds: config.staleLeaseSeconds });
    if (Array.isArray(value)) return Number(value[0] || 0);
    return Number(value || 0);
  }
  async function claim() {
    const rows = await store.rpc('claim_guild_sync_jobs', { p_worker_id: config.workerId, p_limit: config.batchSize });
  ...
  function startHeartbeat(job) {
    ...
    const beat = async () => {
      ...
      inFlight = true;
      try {
        await store.update('guild_sync_jobs', { id: `eq.${job.id}`, status: 'eq.running', claimed_by: `eq.${config.workerId}` }, { updated_at: now().toISOString() }, { returning: false });
      } catch (error) { logger.error(`[guild-sync-worker] heartbeat ${job.id}: ${errorText(error)}`); }
      finally { inFlight = false; }
    };
    const timer = scheduleInterval(beat, config.heartbeatMs);
    return () => { stoppedHeartbeat = true; cancelInterval(timer); };
  }
  async function withHardTimeout(work) {
    let timer;
    const deadline = new Promise((_, reject) => { timer = scheduleTimeout(() => reject(timeoutError(config.jobTimeoutMs)), config.jobTimeoutMs); });
    try { return await Promise.race([work, deadline]); }
    finally { cancelTimeout(timer); }
  }
  async function markCompleted(job, result) {
    await store.update('guild_sync_jobs', { id: `eq.${job.id}`, status: 'eq.running', claimed_by: `eq.${config.workerId}` }, {
      status: 'completed', completed_at: now().toISOString(), sync_run_id: result.syncRunId || null, last_error: null, updated_at: now().toISOString(),
      metadata: {
        ...(job.metadata && typeof job.metadata === 'object' ? job.metadata : {}),
        workerResult: { membersStored: Number(result.membersStored || 0), unitsStored: Number(result.unitsStored || 0), activitySnapshotId: result.activitySnapshotId || null, capturedAt: result.capturedAt || null, boundedTransport: result.boundedTransport === true },
      },
    }, { returning: false });
  }
  async function markFailedOrRetry(job, error) {
    ...
    if (attempts >= maxAttempts) {
      await store.update('guild_sync_jobs', { id: `eq.${job.id}`, status: 'eq.running', claimed_by: `eq.${config.workerId}` }, { status: 'failed', completed_at: timestamp.toISOString(), last_error: message, updated_at: timestamp.toISOString() }, { returning: false });
      return 'failed';
    }
    const delaySeconds = Math.min(3600, config.retryBaseSeconds * (2 ** Math.max(0, attempts - 1)));
    await store.update('guild_sync_jobs', { id: `eq.${job.id}`, status: 'eq.running', claimed_by: `eq.${config.workerId}` }, {
      status: 'queued', run_after: new Date(timestamp.getTime() + delaySeconds * 1000).toISOString(), claimed_at: null, claimed_by: null, last_error: message, completed_at: null, updated_at: timestamp.toISOString(),
    }, { returning: false });
    return 'retry';
  }
  async function markTimedOut(job, error) {
    await store.update('guild_sync_jobs', { id: `eq.${job.id}`, status: 'eq.running', claimed_by: `eq.${config.workerId}` }, { last_error: errorText(error) }, { returning: false });
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  worker.runLoop().catch((error) => { console.error(`[guild-sync-worker] fatal: ${errorText(error)}`); process.exitCode = 1; });
}
